...
- [x] 동기 실행과 비동기 실행 시간을 비교했습니다.
- [x] blocking 엔드포인트의 순차 처리 현상을 재현했습니다.
- [x] blocking 중 헬스체크 지연을 확인했습니다.
- [x] 일반 `def`와 `run_in_executor` 방식을 비교했습니다.
- [x] Day 2의 추론 API에 `run_in_executor`를 적용했습니다.
- [x] 글로벌 예외 핸들러와 요청 로깅 미들웨어를 적용했습니다.
- [x] 1·2·4·8개 동시 요청으로 최종 서버를 검증했습니다.
- [x] 정상·잘못된 크기·잘못된 이미지·헬스체크 요청을 테스트했습니다.
- [x] 각 섹션의 체크포인트 답변을 작성했습니다.
- [ ] GitHub 업로드 시 `SUBMISSION.md`와 `screenshots/` 폴더의 이미지 파일을 함께 업로드합니다.
